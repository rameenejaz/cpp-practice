...
void input(ArtWork allArtWorks[], ArtWork orderCount[], int size) {
    ...
    do {
        ...
        if (!found) {
            cout << "No such Artist in artist list. You must try again!" << '\n';
        }
...
void displayPurchased(ArtWork allArtWorks[], ArtWork orderCount[], int size) {
    cout << left << setw(30) << "Artist" << setw(30) << "Title" << setw(30) << "Size(in sqrt)" << setw(30) << "Price" << '\n';
    for (int i = 0; i < size; i++) {
        // if (orderCount[i]>0) { // Ensure valid purchased items are displayed
        //     cout << left << setw(30) << orderCount[i].Artist;
        //          << setw(30) << orderCount[i].title
        //          << setw(30) << (orderCount[i].height * orderCount[i].width)
        //          << "$" << fixed << setprecision(2) << setw(30) << orderCount[i].price << '\n';
        }
    }
// }
